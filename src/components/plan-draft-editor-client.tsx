...
function newDraftAction(index: number, leadDepartmentId: string, ownerUserId: string, endDate: string): PlanDraftAction {
  return { client_id: `draft-ui-${Date.now()}-${index}`, title: "", description: null, priority: "NORMAL", lead_department_id: leadDepartmentId || null, collaborating_department_ids: [], collaborating_group_ids: [], collaborating_user_ids: [], parent_client_id: null, assignment_target_type: "USER", assignee_user_id: ownerUserId || null, assignee_group_id: null, start_date: null, due_date: endDate || null, expected_result: "", verification_requirement: null, milestone_group: null, is_required: true, criteria_refs: [], automation_kind: "ACTION", automation_confirmed: false, automation_outputs: [], automation_ref_id: null, automation_target_department_id: null, automation_target_area: null, automation_report_recipient: null, automation_report_method: null, automation_report_period: null, automation_report_recurrence_rule: null, automation_report_recurrence_end_date: null, automation_assessment_round_type: null, automation_audit_type: null };
}
...
export function PlanDraftEditorClient({ planId, canManage, status, initial, departments, profiles }: { planId: string; canManage: boolean; status: string; initial: Initial; departments: Department[]; profiles: Profile[] }) {
  ...
  async function save(event: FormEvent) {
    ...
    if (!title.trim() || !generalObjective.trim() || !leadDepartmentId) { setError("Cần hoàn thiện tên kế hoạch, mục tiêu chung và khoa/phòng chủ trì."); return; }
    ...
    for (let index = 0; index < draftActions.length; index++) {
      ...
      if (!action.title.trim() || !action.lead_department_id || (action.assignment_target_type === "USER" ? !action.assignee_user_id : !action.assignee_group_id) || !action.due_date || !action.expected_result.trim()) { setError(`Nhiệm vụ #${index + 1} cần đủ nội dung, khoa/phòng, đối tượng được giao, hạn và kết quả mong đợi.`); return; }
  ...
  return <section className="panel plan-composer-editor">
    ...
    {open ? <form className="pc-body" onSubmit={save}>
      ...
      <section><h3>2. Mục tiêu & yêu cầu</h3><div className="pc-grid"><label className="wide">Mục tiêu chung *<textarea rows={3} value={generalObjective} onChange={(e) => setGeneralObjective(e.target.value)} /></label><div className="wide" style={{ display: "grid", gap: 8 }}><strong style={{ fontSize: 11 }}>Mục tiêu cụ thể (không bắt buộc)</strong>{specificObjectives.map((item, index) => <div className="specific-row" key={index}><input value={item} onChange={(e) => setSpecific(index, e.target.value)} placeholder={`Mục tiêu ${index + 1}`} /><button type="button" className="button tertiary small" onClick={() => removeSpecific(index)}>Xóa</button></div>)}<div><button type="button" className="button secondary small" onClick={addSpecific}>+ Thêm mục tiêu</button></div></div><label className="wide">Yêu cầu (không bắt buộc)<textarea rows={3} value={requirements} onChange={(e) => setRequirements(e.target.value)} /></label><label className="wide">Mô tả / phạm vi<textarea rows={3} value={description} onChange={(e) => setDescription(e.target.value)} /></label></div></section>
