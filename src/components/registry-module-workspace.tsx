...
type Row={id:string;record_type:string;record_code:string;title:string;lifecycle_status:string;department_name:string;owner_name:string;updated_at:string;route:string};
...
export function RegistryModuleWorkspace(p:{year:number;rows:Row[];workflow:string[];principle:string;related:{label:string;href:string}[];operatingSpec?:ModuleOperatingSpec|null}){
 ...
 return <>
  ...
  {p.operatingSpec?<section className="operating-spec-panel">
  ...
  <section className="panel module-list-panel" id="registry-list"><div className="module-toolbar"><input value={q} onChange={e=>{const value=e.target.value;setQ(value);syncUrl(value,status)}} placeholder="Tìm mã, tên, đơn vị, người phụ trách..."/><select value={status} onChange={e=>{const value=e.target.value;setStatus(value);syncUrl(q,value)}}><option value="ALL">Tất cả trạng thái</option><option value="ACTIVE">Đang hoạt động</option><option value="CLOSED">Đã đóng</option><option value="CANCELLED">Đã hủy</option><option value="ARCHIVED">Lưu trữ</option></select></div><div className="module-list-summary"><strong>{rows.length}</strong> / {p.rows.length} hồ sơ · năm {p.year}</div>
