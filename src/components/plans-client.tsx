...
export function PlansClient({ year, canManage, rows, departments, profiles }: { year: number; canManage: boolean; rows: PlanRow[]; departments: Department[]; profiles: Profile[] }) {
  ...
  const filtered = rows.filter((row) => {
    const text = `${row.record_code} ${row.title} ${deptMap.get(row.lead_department_id || "") || ""}`.toLowerCase();
    return text.includes(search.trim().toLowerCase()) && (status === "ALL" || row.workflow_status === status);
  });
  ...
  function requestClose() {
    if (busy) return;
    const changed = JSON.stringify(form) !== JSON.stringify(initialForm(year));
  ...
  async function submit(e: FormEvent) {
    ...
    if (!form.objective.trim()) return setMessage({ tone: "error", text: "Vui lòng nhập mục tiêu kế hoạch." });
    ...
    try {
      ...
      setMessage({ tone: "success", text: `Đã tạo kế hoạch ${data.record_code}.` });
      router.refresh();
      setTimeout(() => {
        setFormOpen(false);
        if (data.id) router.push(`/plans/${data.id}`);
      }, 500);
    } catch (error) {
    ...
    <div className="modal-backdrop" style={{ padding: 16 }}>
      ...
      >
        <div className="modal-head" style={{ flexShrink: 0, padding: "18px 24px" }}>
          <div>
            <div className="eyebrow">KẾ HOẠCH · {year}</div>
            <h2>Tạo kế hoạch mới</h2>
            <p className="muted" style={{ margin: "5px 0 0", fontSize: 12 }}>
              Khai báo thông tin nền, đơn vị chủ trì, thời gian và mục tiêu của kế hoạch.
            </p>
          </div>
          ...
          <div className="form-stack" style={{ gap: 24 }}>
            <section>
              <div style={{ marginBottom: 13 }}>
                <strong style={{ fontSize: 14 }}>1. Thông tin kế hoạch</strong>
                <div className="muted tiny" style={{ marginTop: 4 }}>Tên và loại kế hoạch được dùng xuyên suốt trong hệ thống.</div>
              </div>
              <div className="form-grid two" style={{ gap: 16 }}>
                <label className="span-2">
                  <span>Tên kế hoạch *</span>
                  <input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder={`Ví dụ: Kế hoạch hoạt động quản lý chất lượng bệnh viện năm ${year}`} />
                </label>
                <label>
                ...
                <label>
                  <span>Khoa/Phòng chủ trì *</span>
                  <select value={form.leadDepartmentId} onChange={(e) => setForm({ ...form, leadDepartmentId: e.target.value, ownerUserId: "" })}>
            ...
            <section style={{ borderTop: "1px solid var(--line)", paddingTop: 22 }}>
              <div style={{ marginBottom: 13 }}>
                <strong style={{ fontSize: 14 }}>2. Phụ trách & thời gian</strong>
                <div className="muted tiny" style={{ marginTop: 4 }}>Xác định đầu mối và thời gian dự kiến triển khai.</div>
              </div>
              <div className="form-grid two" style={{ gap: 16 }}>
                <label>
                ...
                <label className="span-2">
                  ...
                  <small>Có thể để trống và phân công sau.</small>
                </label>
              </div>
            </section>
            ...
            <section style={{ borderTop: "1px solid var(--line)", paddingTop: 22 }}>
              <div style={{ marginBottom: 13 }}>
                <strong style={{ fontSize: 14 }}>3. Mục tiêu & phạm vi</strong>
                <div className="muted tiny" style={{ marginTop: 4 }}>Nêu rõ mục tiêu và phạm vi để làm căn cứ triển khai các Action/Task.</div>
              </div>
              <div className="form-grid two" style={{ gap: 16 }}>
                <label className="span-2">
                  <span>Mục tiêu *</span>
                  <textarea rows={4} value={form.objective} onChange={(e) => setForm({ ...form, objective: e.target.value })} placeholder="Mục tiêu chính của kế hoạch" />
                </label>
                <label className="span-2">
                  <span>Mô tả / phạm vi</span>
                  <textarea rows={5} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Nội dung, phạm vi áp dụng hoặc ghi chú triển khai" />
                </label>
              </div>
            </section>

            <div className="scope-note">
              Kế hoạch mới được tạo ở trạng thái <strong>Nháp</strong>. Các nhiệm vụ cụ thể được quản lý bằng Action/Task và liên kết trực tiếp vào kế hoạch.
            </div>
          </div>
        </div>
        ...
        <div className="modal-footer" style={{ flexShrink: 0, padding: "14px 24px", boxShadow: "0 -6px 18px rgba(26,42,49,.04)" }}>
          ...
          <button className="button primary" disabled={busy}><Icon name="save" size={17} /> {busy ? "Đang tạo..." : "Tạo kế hoạch"}</button>
        </div>
      </form>
    </div>,
    document.body,
  ...
  return <>
    <section className="kpi-grid">
      <article className="kpi-card"><span>Tổng kế hoạch</span><strong>{total}</strong><small>Trong năm {year}</small></article>
      <article className="kpi-card"><span>Đang triển khai</span><strong>{inProgress}</strong><small>Đã duyệt / đang thực hiện</small></article>
      <article className="kpi-card danger"><span>Có việc quá hạn</span><strong>{overdue}</strong><small>Kế hoạch cần chú ý</small></article>
      <article className="kpi-card success"><span>Hoàn thành</span><strong>{completed}</strong><small>Kế hoạch đã hoàn tất</small></article>
    </section>

    <section className="panel">
      <div className="toolbar">
        <div className="toolbar-left">
          <div className="search-box"><Icon name="search" size={18} /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Tìm mã, tên kế hoạch, khoa/phòng..." /></div>
          <select style={{ width: 190 }} value={status} onChange={(e) => setStatus(e.target.value)}><option value="ALL">Tất cả trạng thái</option><option value="DRAFT">Nháp</option><option value="PENDING_APPROVAL">Chờ phê duyệt</option><option value="APPROVED">Đã phê duyệt</option><option value="IN_PROGRESS">Đang thực hiện</option><option value="ON_HOLD">Tạm dừng</option><option value="COMPLETED">Hoàn thành</option><option value="CANCELLED">Đã hủy</option></select>
        </div>
        {canManage ? <button className="button primary" onClick={openCreate}><Icon name="plus" size={18} /> Tạo kế hoạch</button> : null}
      </div>
      <div className="table-wrap"><table><thead><tr><th>Mã</th><th>Kế hoạch</th><th>Loại</th><th>Chủ trì</th><th>Thời gian</th><th>Tiến độ</th><th>Trạng thái</th></tr></thead><tbody>
        {filtered.map((row) => { const pct = Math.max(0, Math.min(100, Math.round(row.progress_pct || 0))); return <tr key={row.id}><td><Link className="table-link" href={`/plans/${row.id}`}>{row.record_code}</Link></td><td><Link href={`/plans/${row.id}`}><strong>{row.title}</strong></Link></td><td>{TYPE_LABELS[row.program_type] || row.program_type}</td><td>{deptMap.get(row.lead_department_id || "") || "—"}{row.owner_user_id ? <span className="subline">{userMap.get(row.owner_user_id) || "Người phụ trách"}</span> : null}</td><td>{formatDate(row.start_date)}<span className="subline">đến {formatDate(row.end_date)}</span></td><td><div className="progress-cell"><div className="progress-track"><span style={{ width: `${pct}%` }} /></div><strong>{pct}%</strong></div><span className="subline">{row.completed_actions}/{row.required_actions} việc · {row.overdue_actions} quá hạn</span></td><td><StatusBadge status={row.workflow_status} /></td></tr>; })}
        {!filtered.length ? <tr><td colSpan={7}><div className="empty-state">Chưa có kế hoạch phù hợp trong năm {year}.</div></td></tr> : null}
      </tbody></table></div>
    </section>
    {modal}
  </>;
