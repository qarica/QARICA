...
export async function RecordActionsPanel({ recordId, recordType, sourceTitle, allowCreate = true }: { recordId: string; recordType: string; sourceTitle: string; allowCreate?: boolean }) {
  ...
  const [recordRes, actionRes, departmentsRes, profilesRes, groupsRes] = await Promise.all([
    ...
    targetIds.length ? supabase.from("actions").select("record_id,priority,due_date,workflow_status,assignment_target_type,assignee_user_id,assignee_group_id").in("record_id", targetIds) : Promise.resolve({ data: [], error: null }),
    ...
    canCreate || targetIds.length ? supabase.from("work_groups").select("id,code,name,lead_department_id,leader_user_id,is_active").eq("is_active", true).order("name") : Promise.resolve({ data: [], error: null }),
  ] as any);
  ...
  for (const g of (groupsRes.data ?? []) as any[]) groupMap.set(String(g.id), [g.code, g.name].filter(Boolean).join(" · "));
