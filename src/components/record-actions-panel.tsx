...
export async function RecordActionsPanel({ recordId, recordType, sourceTitle }: { recordId: string; recordType: string; sourceTitle: string }) {
  ...
  return <section className="panel linked-actions-panel">
    ...
    <div className="lap-head"><div><h2>Hành động / nhiệm vụ liên kết</h2><p>Biến yêu cầu, phát hiện hoặc vấn đề trong hồ sơ thành Action có owner, deadline, minh chứng và bước xác minh. Action đã hủy được tách khỏi màn hình vận hành.</p></div>{canCreate ? <RecordActionCreateClient recordId={recordId} recordType={recordType} sourceTitle={sourceTitle} departments={(departmentsRes.data ?? []) as any[]} profiles={(profilesRes.data ?? []) as any[]} /> : null}</div>
