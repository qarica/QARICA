...
type FormState = { id: string; name: string; sourceCode: string; description: string; ownerDepartmentId: string; scoringMethod: string };

const initialForm: FormState = { id: "", name: "", sourceCode: "", description: "", ownerDepartmentId: "", scoringMethod: "COMPLIANCE_PERCENTAGE" };
...
export function MonitoringClient({ year, canManageTemplates, templateRows, monitoringRows, departments }: {
  ...
  function closeCreate() {
    if (busy) return;
    const changed = form.id ? true : JSON.stringify(form) !== JSON.stringify(initialForm);
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      const res = await fetch(isEdit ? `/api/monitoring/templates/${form.id}` : "/api/monitoring/templates", { method: isEdit ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: form.name.trim(), source_code: form.sourceCode.trim() || null, description: form.description.trim() || null, owner_department_id: form.ownerDepartmentId, scoring_method: form.scoringMethod }) });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || (isEdit ? "Không thể cập nhật mẫu bảng kiểm." : "Không thể tạo mẫu bảng kiểm."));
      setMessage({ tone: "success", text: isEdit ? "Đã cập nhật thông tin mẫu bảng kiểm." : `Đã tạo ${data.code} · Phiên bản 1 ở trạng thái Nháp.` });
      router.refresh();
      setTimeout(() => { setFormOpen(false); if (!isEdit && data.id) router.push(`/monitoring/templates/${data.id}`); }, 450);
    } catch (error) {
  ...
  const modal = formOpen && typeof document !== "undefined" ? createPortal(
    <div className="modal-backdrop" style={{ padding: 16 }}>
      <form className="modal-card" onSubmit={submit} style={{ width: "min(900px, calc(100vw - 32px))", maxHeight: "calc(100dvh - 32px)", borderRadius: 18 }}>
        <div className="modal-head" style={{ padding: "18px 24px" }}><div><div className="eyebrow">MẪU BẢNG KIỂM</div><h2>{form.id ? "Cập nhật mẫu bảng kiểm" : "Tạo mẫu bảng kiểm mới"}</h2><p className="muted" style={{ margin: "5px 0 0", fontSize: 12 }}>{form.id ? "Chỉ cập nhật thông tin master; nội dung phiên bản đã phát hành vẫn được khóa." : <>QARICA tự sinh <strong>Mã nội bộ BK-YYYY-####</strong>; mã biểu mẫu nguồn được lưu riêng để truy vết.</>}</p></div><button type="button" className="icon-button" aria-label="Đóng" onClick={closeCreate}><Icon name="x" size={22} /></button></div>
        <div className="modal-body" style={{ padding: "24px 28px 30px" }}>{message ? <div className={`alert ${message.tone}`} style={{ marginBottom: 18 }}>{message.text}</div> : null}<div className="form-stack"><label><span>Tên mẫu bảng kiểm *</span><input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Ví dụ: Bảng kiểm tuân thủ vệ sinh tay" /></label><label><span>Mã nguồn / mã biểu mẫu</span><input value={form.sourceCode} onChange={(e) => setForm({ ...form, sourceCode: e.target.value })} placeholder="Ví dụ: QĐ.QLCL.02-BK01 (nếu văn bản nguồn có mã)" /><small>Mã này phục vụ truy vết nguồn. Mã QARICA được sinh tự động và không chứa phiên bản.</small></label><label><span>Khoa/Phòng quản lý mẫu *</span><select value={form.ownerDepartmentId} onChange={(e) => setForm({ ...form, ownerDepartmentId: e.target.value })}><option value="">— Chọn đơn vị —</option>{departments.map((d) => <option key={d.id} value={d.id}>{d.name}</option>)}</select></label>{!form.id ? <label><span>Phương pháp tổng hợp kết quả</span><select value={form.scoringMethod} onChange={(e) => setForm({ ...form, scoringMethod: e.target.value })}><option value="COMPLIANCE_PERCENTAGE">Tỷ lệ tuân thủ (%)</option><option value="WEIGHTED_SCORE">Điểm có trọng số</option><option value="NO_SCORE">Không tính điểm</option></select><small>Có thể cấu hình điểm/trọng số chi tiết ở từng tiêu chí sau.</small></label> : null}<label><span>Mô tả / phạm vi sử dụng</span><textarea rows={4} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Mô tả mục đích, phạm vi, đối tượng áp dụng..." /></label></div></div>
        <div className="modal-footer" style={{ padding: "14px 24px" }}><button type="button" className="button secondary" disabled={busy} onClick={closeCreate}>Hủy / Đóng</button><button className="button primary" disabled={busy}><Icon name="plus" size={17} /> {busy ? "Đang lưu..." : form.id ? "Lưu thay đổi" : "Tạo mẫu bảng kiểm"}</button></div>
      </form>
    </div>, document.body,
  ) : null;
  ...
  return <>
    <style>{`
      ...
      @media(max-width:700px){
        ...
        .monitoring-round-card:active{background:#f8fafc}.monitoring-round-card-head{display:flex;justify-content:space-between;gap:8px;align-items:flex-start}.monitoring-round-card-code{font-size:13px;font-weight:800;color:#1d3f73}.monitoring-round-card-title{font-size:16px;font-weight:800;line-height:1.35;color:#17333a}.monitoring-round-card-meta{display:flex;gap:7px;flex-wrap:wrap;color:#69777d;font-size:11px;line-height:1.35}.monitoring-round-card-area{font-size:12px;color:#58686e;line-height:1.4}.monitoring-round-card-result{display:flex;justify-content:space-between;align-items:center;gap:8px;padding-top:8px;border-top:1px solid #edf2f2;font-size:11px;color:#5d6d72}.monitoring-round-card-result strong{color:#1d3f73;font-size:13px}.monitoring-round-card-result.muted{color:#889599}
      }
    `}</style>
    ...
    <section className="panel">
      ...
      {tab === "templates" ? <div className="table-wrap"><table><thead><tr><th>Mã QARICA</th><th>Tên mẫu bảng kiểm</th><th>Đơn vị quản lý</th><th>Phiên bản mới nhất</th><th>Cấu trúc</th><th>Cách tính</th><th></th></tr></thead><tbody>{filteredTemplates.map((row) => <tr key={row.id}><td><Link className="table-link" href={`/monitoring/templates/${row.id}`}>{row.code || "—"}</Link>{row.source_code ? <span className="subline">Nguồn: {row.source_code}</span> : null}</td><td><Link className="table-link" href={`/monitoring/templates/${row.id}`}>{row.name}</Link>{!row.is_active ? <span className="subline text-danger">Đã ngưng sử dụng</span> : null}</td><td>{deptMap.get(row.owner_department_id || "") || "—"}</td><td><div style={{ display: "flex", gap: 7, alignItems: "center", flexWrap: "wrap" }}><strong>{row.latest_version_no ? `v${row.latest_version_no}` : "—"}</strong>{row.latest_version_status ? <VersionBadge status={row.latest_version_status} /> : null}</div>{row.effective_from ? <span className="subline">Hiệu lực từ {formatDate(row.effective_from)}</span> : null}</td><td><strong>{row.section_count}</strong> nhóm · <strong>{row.item_count}</strong> tiêu chí</td><td>{scoringLabel(row.scoring_method)}</td><td>{canManageTemplates ? <div style={{display:"flex",gap:5,justifyContent:"flex-end"}}><button className="button tertiary small" onClick={()=>openEdit(row)}>Cập nhật</button><button className="button secondary small" disabled={busy} onClick={async()=>{if(!window.confirm(row.is_active?"Ngưng sử dụng mẫu bảng kiểm này? Các đợt giám sát cũ vẫn được giữ.":"Kích hoạt lại mẫu bảng kiểm này?"))return;setBusy(true);try{const res=await fetch(`/api/monitoring/templates/${row.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({is_active:!row.is_active})});const data=await res.json();if(!res.ok)throw new Error(data.error||"Không cập nhật được trạng thái.");router.refresh()}catch(error){alert(error instanceof Error?error.message:"Có lỗi xảy ra.")}finally{setBusy(false)}}}>{row.is_active?"Ngưng":"Kích hoạt"}</button></div> : null}</td></tr>)}{!filteredTemplates.length ? <tr><td colSpan={7}><div className="empty-state">Chưa có mẫu bảng kiểm.</div></td></tr> : null}</tbody></table></div> : <>
        <div className="monitoring-round-desktop table-wrap"><table><thead><tr><th>Mã đợt</th><th>Đợt giám sát</th><th>Bảng kiểm</th><th>Khoa/Phòng</th><th>Ngày</th><th>Kết quả</th><th>Trạng thái</th></tr></thead><tbody>{filteredRounds.map((row) => <tr key={row.id}><td><Link className="table-link" href={`/monitoring/${row.id}`}><strong>{row.record_code}</strong></Link></td><td><Link className="table-link" href={`/monitoring/${row.id}`}><strong>{row.title}</strong></Link>{row.target_area ? <span className="subline">Khu vực: {row.target_area}</span> : null}</td><td>{row.checklist_name}</td><td>{deptMap.get(row.target_department_id || "") || "—"}</td><td>{formatDate(row.scheduled_date)}</td><td>{row.scored > 0 ? <><strong style={{ color: "#1d3f73" }}>{row.pass_pct}%</strong><span className="subline">{row.fail} không đạt</span></> : <span className="muted">Chưa chấm</span>}</td><td><MonitoringPhaseBadge phase={row.phase} /></td></tr>)}{!filteredRounds.length ? <tr><td colSpan={7}><div className="empty-state">Không có đợt giám sát phù hợp bộ lọc hiện tại.</div></td></tr> : null}</tbody></table></div>
        <div className="monitoring-round-mobile">{filteredRounds.map((row) => <Link key={row.id} href={`/monitoring/${row.id}`} className="monitoring-round-card"><div className="monitoring-round-card-head"><span className="monitoring-round-card-code">{row.record_code}</span><MonitoringPhaseBadge phase={row.phase} /></div><div className="monitoring-round-card-title">{row.title}</div><div className="monitoring-round-card-meta"><span>{formatDate(row.scheduled_date)}</span><span>•</span><span>{row.checklist_name}</span>{deptMap.get(row.target_department_id || "") ? <><span>•</span><span>{deptMap.get(row.target_department_id || "")}</span></> : null}</div>{row.scored > 0 ? <div className="monitoring-round-card-result"><strong>{row.pass_pct}% đạt</strong><span>{row.pass} đạt · {row.fail} không đạt</span></div> : <div className="monitoring-round-card-result muted">Chưa có mục được chấm</div>}{row.target_area ? <div className="monitoring-round-card-area">Khu vực: {row.target_area}</div> : null}</Link>)}{!filteredRounds.length ? <div className="empty-state">Không có đợt giám sát phù hợp bộ lọc hiện tại.</div> : null}</div>
      </>}
    </section>
    {modal}
  </>;
