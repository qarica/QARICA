...
type FormState = { name: string; description: string; sourceCode: string; ownerDepartmentId: string; scoringMethod: string };

const initialForm: FormState = { name: "", description: "", sourceCode: "", ownerDepartmentId: "", scoringMethod: "COMPLIANCE_PERCENTAGE" };
...
export function MonitoringClient({ year, canManageTemplates, templateRows, monitoringRows, departments }: {
  ...
  const filteredTemplates = templateRows.filter((row) => `${row.internal_code || ""} ${row.code || ""} ${row.name} ${row.description || ""} ${deptMap.get(row.owner_department_id || "") || ""}`.toLowerCase().includes(q));
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      const res = await fetch("/api/monitoring/templates", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: form.name.trim(), description: form.description.trim() || null, source_code: form.sourceCode.trim() || null, owner_department_id: form.ownerDepartmentId, scoring_method: form.scoringMethod }) });
