...
export async function DomainWorkflowPanel({ recordId, recordType }: { recordId: string; recordType: string }) {
  ...
  if (recordType === "FMEA") {
    const {data:study}=await supabase.from("fmea_studies").select("id,workflow_status,scoring_model_version_id").eq("record_id",recordId).maybeSingle();if(!study)return null;
    const[{data:steps},{data:links},{count:evidence},{data:departments}]=await Promise.all([supabase.from("fmea_process_steps").select("id,step_no,step_name,description,responsible_department_id").eq("fmea_study_id",study.id).order("step_no"),supabase.from("record_links").select("target_record_id").eq("source_record_id",recordId).eq("relation_type","HAS_ACTION"),supabase.from("evidence_links").select("id",{count:"exact",head:true}).eq("record_id",recordId),supabase.from("departments").select("id,name,short_name").eq("is_active",true).order("name")]);
    const stepIds=(steps??[]).map((x:any)=>x.id);const{data:modes}=stepIds.length?await supabase.from("fmea_failure_modes").select("id,process_step_id,failure_mode,potential_effect,potential_cause,current_control,is_high_priority").in("process_step_id",stepIds):{data:[] as any[]};const ids=(links??[]).map((x:any)=>x.target_record_id).filter(Boolean);const{data:actions}=ids.length?await supabase.from("actions").select("record_id,workflow_status").in("record_id",ids):{data:[] as any[]};const active=(actions??[]).filter((x:any)=>!["CANCELLED","NOT_APPLICABLE"].includes(String(x.workflow_status)));
    const departmentMap=new Map((departments??[]).map((x:any)=>[x.id,x.short_name||x.name]));const stepMap=new Map((steps??[]).map((x:any)=>[x.id,`${x.step_no}. ${x.step_name}`]));
    return <FmeaWorkflowClient recordId={recordId} status={study.workflow_status} canManage={user.permissions.includes("risk.manage")} hasModel={!!study.scoring_model_version_id} steps={stepIds.length} modes={(modes??[]).length} highPriority={(modes??[]).filter((x:any)=>x.is_high_priority).length} actions={active.length} incomplete={active.filter((x:any)=>x.workflow_status!=="COMPLETED").length} evidence={evidence??0} departments={(departments??[]).map((x:any)=>({id:x.id,label:x.short_name||x.name}))} stepRows={(steps??[]).map((x:any)=>({id:x.id,no:x.step_no,name:x.step_name,description:x.description||"",department:departmentMap.get(x.responsible_department_id)||""}))} modeRows={(modes??[]).map((x:any)=>({id:x.id,step:stepMap.get(x.process_step_id)||"",mode:x.failure_mode,effect:x.potential_effect||"",cause:x.potential_cause||"",control:x.current_control||"",high:!!x.is_high_priority}))}/>;
  }
