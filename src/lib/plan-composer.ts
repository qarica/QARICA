...
export type PlanDraftAction = {
  ...
  assignment_target_type: "DEPARTMENT" | "USER" | "GROUP";
...
export function cleanPlanDraftActions(value: unknown): PlanDraftAction[] {
  ...
  return value.slice(0, 300).map((raw: any, index) => {
    ...
    return {
      ...
      assignment_target_type: planText(raw?.assignment_target_type).toUpperCase() === "GROUP" || planText(raw?.assignee_group_id)
        ? "GROUP"
        : planText(raw?.assignment_target_type).toUpperCase() === "USER" || planText(raw?.assignee_user_id)
          ? "USER"
          : "DEPARTMENT",
