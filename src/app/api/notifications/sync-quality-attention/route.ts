...
export async function POST() {
  ...
  for (const row of (directivesRes.data ?? []) as any[]) {
    ...
    if (!phase) continue;
    add(row.record_id, `DIRECTIVE_${phase.phase}`, phase.priority, phase.phase === "OVERDUE" ? "Chỉ đạo/Yêu cầu quá hạn" : "Chỉ đạo/Yêu cầu sắp đến hạn", dueMessage(phase.phase, due, "Chỉ đạo/Yêu cầu"), `quality:directive:${row.id}:${phase.phase}:due:${String(due).slice(0,10)}`);
  }
  ...
  for (const row of (reportsRes.data ?? []) as any[]) {
    ...
    add(row.record_id, `REPORT_${phase.phase}`, phase.priority, phase.phase === "OVERDUE" ? "Báo cáo quá hạn" : "Báo cáo sắp đến hạn", `${dueMessage(phase.phase, row.due_date, "Báo cáo")}${recipient}`, `quality:report:${row.id}:${phase.phase}:due:${String(row.due_date).slice(0,10)}`);
  }
  ...
  for (const row of (risksRes.data ?? []) as any[]) {
    ...
    if (!phase) continue;
    add(row.record_id, `RISK_REVIEW_${phase.phase}`, phase.priority, phase.phase === "OVERDUE" ? "Rủi ro quá hạn rà soát" : "Rủi ro đến kỳ rà soát", dueMessage(phase.phase, row.next_review_date, "Kỳ rà soát rủi ro"), `quality:risk:${row.id}:${phase.phase}:review:${String(row.next_review_date).slice(0,10)}`);
  }
  ...
  if (canManageCapa) {
    for (const row of (capasRes.data ?? []) as any[]) {
      ...
      if (row.workflow_status === "EFFECTIVENESS_REVIEW") {
        ...
        const phase = phaseFor(row.effectiveness_due_date, today, ATTENTION_WINDOW_DAYS.capa);
        if (phase) add(row.record_id, `CAPA_EFFECTIVENESS_${phase.phase}`, phase.priority, "CAPA gần hạn đánh giá hiệu lực", dueMessage(phase.phase, row.effectiveness_due_date, "Đánh giá hiệu lực CAPA"), `quality:capa:${row.id}:${phase.phase}:effectiveness:${String(row.effectiveness_due_date).slice(0,10)}`);
      }
    }
  }
  ...
  if (canManageFeedback) {
    for (const row of (feedbackRes.data ?? []) as any[]) {
      ...
      if (!phase) continue;
      add(row.record_id, `FEEDBACK_RESPONSE_${phase.phase}`, phase.priority, phase.phase === "OVERDUE" ? "Phản ánh quá hạn phản hồi" : "Phản ánh cần phản hồi", dueMessage(phase.phase, row.response_due_at, "Hạn phản hồi"), `quality:feedback:${row.id}:${phase.phase}:response:${String(row.response_due_at).slice(0,10)}`);
    }
  }
  ...
  if (canManageInspections) {
    for (const row of (inspectionsRes.data ?? []) as any[]) {
      ...
      add(row.record_id, `INSPECTION_${phase.phase}`, phase.priority, phase.phase === "OVERDUE" ? "Đợt kiểm tra chưa hoàn tất sau ngày đoàn" : "Tiếp đoàn sắp tới", `${dueMessage(phase.phase, row.visit_date, "Ngày đoàn đến")}${authority}`, `quality:inspection:${row.id}:${phase.phase}:visit:${String(row.visit_date).slice(0,10)}`);
    }
  }
  ...
  for (const row of (findingsRes.data ?? []) as any[]) {
    ...
    add(row.record_id, `FINDING_${phase.phase}`, phase.priority, phase.phase === "OVERDUE" ? "Finding quá hạn" : "Finding sắp đến hạn", `${dueMessage(phase.phase, row.due_date, "Finding")}${severity}`, `quality:finding:${row.id}:${phase.phase}:due:${String(row.due_date).slice(0,10)}`);
  }
