...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: record } = await supabase.from("records").select("id,lifecycle_status").eq("id", recordId).eq("record_type", "IMPROVEMENT_PROJECT").maybeSingle();
  ...
  const { data: project, error } = await admin.from("improvement_projects").select("id,workflow_status,problem_statement,start_date,target_end_date,scope_description").eq("record_id", recordId).maybeSingle();
  ...
  if (command === "SUBMIT") {
    ...
    if (recordError) {
      await admin.from("improvement_projects").update({ workflow_status: oldStatus, actual_end_date: null, updated_at: now }).eq("id", project.id);
      return NextResponse.json({ error: recordError.message }, { status: 400 });
    }
    const { error: historyError } = await admin.from("record_status_history").insert({ record_id: recordId, old_status: "ACTIVE", new_status: "CLOSED", changed_by: auth.user.id, reason });
    if (historyError) {
      await admin.from("records").update({ lifecycle_status: "ACTIVE", closed_at: null, updated_at: now }).eq("id", recordId);
      await admin.from("improvement_projects").update({ workflow_status: oldStatus, actual_end_date: null, updated_at: now }).eq("id", project.id);
  ...
  await admin.from("audit_logs").insert({ actor_user_id: auth.user.id, record_id: recordId, table_name: "improvement_projects", row_id: project.id, action_type: `IMPROVEMENT_PROJECT_${command}`, old_value: { workflow_status: oldStatus }, new_value: { workflow_status: newStatus }, reason, request_meta: { source: "qlcl-ui", transaction } });
