...
import { arePdsaMilestonesComplete, isPdsaLearningComplete, normalizeProjectMilestone, normalizeProjectObjective } from "@/lib/improvement-project-setup";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: record } = await supabase.from("records").select("id,lifecycle_status,closed_at,updated_at").eq("id", recordId).eq("record_type", "IMPROVEMENT_PROJECT").maybeSingle();
  ...
  const { data: project, error } = await admin.from("improvement_projects").select("id,workflow_status,problem_statement,start_date,target_end_date,scope_description,approved_at,actual_end_date,updated_at").eq("record_id", recordId).maybeSingle();
  ...
  const [{ data: objectiveRows }, { data: milestoneRows }, { data: links }, { count: evidence }] = await Promise.all([
    admin.from("project_objectives").select("*").eq("project_id", project.id),
    admin.from("project_milestones").select("*").eq("project_id", project.id),
  ...
  const objectives = (objectiveRows || []).map((row: any, index: number) => normalizeProjectObjective(row, index));
  const milestones = (milestoneRows || []).map((row: any, index: number) => normalizeProjectMilestone(row, index));
  ...
  if (command === "SUBMIT") {
    ...
    if (!objectives.length || !milestones.length) return NextResponse.json({ error: "Cần ít nhất 01 mục tiêu SMART và 01 milestone/PDSA." }, { status: 409 });
    const incompleteSmart = objectives.filter((item: any) => !item.statement || !item.indicator || !item.baseline || !item.target || !item.due_date);
    if (incompleteSmart.length) return NextResponse.json({ error: `Còn ${incompleteSmart.length} mục tiêu chưa đủ cấu trúc SMART (chỉ số, baseline, target, hạn đạt).` }, { status: 409 });
    const phases = new Set(milestones.map((item: any) => item.phase));
    const missingPhases = ["PLAN", "DO", "STUDY", "ACT"].filter((phase) => !phases.has(phase));
    if (missingPhases.length) return NextResponse.json({ error: `Chu trình PDSA chưa đủ pha: ${missingPhases.join(", ")}.` }, { status: 409 });
    ...
    const milestoneStatuses = milestones.map((item: any) => item.status);
    ...
    if (!isPdsaLearningComplete(milestones)) return NextResponse.json({ error: "Chưa đủ bằng chứng học tập PDSA: cần ít nhất 01 STUDY có kết quả + bài học và 01 ACT có quyết định Adopt/Adapt/Abandon." }, { status: 409 });
    ...
    const { error: updateError } = await admin.from("improvement_projects").update({ workflow_status: newStatus, updated_at: now }).eq("id", project.id);
    if (updateError) { await admin.from("project_closure_reviews").delete().eq("id", review.id); return NextResponse.json({ error: updateError.message }, { status: 400 }); }
    projectChanged = true;
    ...
    const { data: tx, error: txError } = await admin.rpc(CLOSE_PROJECT_RPC, { p_project_record_id: recordId, p_actor_user_id: auth.user.id, p_reason: reason });
    ...
    if (recordError) { await admin.from("improvement_projects").update(projectRollback).eq("id", project.id); return NextResponse.json({ error: recordError.message }, { status: 400 }); }
    recordChanged = true;
    ...
    if (historyError || !history) { await admin.from("records").update(recordRollback).eq("id", recordId); await admin.from("improvement_projects").update(projectRollback).eq("id", project.id); return NextResponse.json({ error: historyError?.message || "Không ghi được lịch sử trạng thái đề án." }, { status: 400 }); }
  ...
  } else return NextResponse.json({ error: "Thao tác đề án không hợp lệ." }, { status: 400 });

  const { error: auditError } = await admin.from("audit_logs").insert({ actor_user_id: auth.user.id, record_id: recordId, table_name: "improvement_projects", row_id: project.id, action_type: `IMPROVEMENT_PROJECT_${command}`, old_value: { workflow_status: oldStatus, approved_at: project.approved_at ?? null, actual_end_date: project.actual_end_date ?? null }, new_value: { workflow_status: newStatus, ...auditDetails }, reason, request_meta: { source: "qlcl-ui", transaction } });
  if (auditError) {
    const rollbackErrors: string[] = [];
    if (createdReviewId) { const { error: e } = await admin.from("project_closure_reviews").delete().eq("id", createdReviewId); if (e) rollbackErrors.push(`review: ${e.message}`); }
    if (createdHistoryId) { const { error: e } = await admin.from("record_status_history").delete().eq("id", createdHistoryId); if (e) rollbackErrors.push(`history: ${e.message}`); }
    if (recordChanged) { const { error: e } = await admin.from("records").update(recordRollback).eq("id", recordId); if (e) rollbackErrors.push(`record: ${e.message}`); }
    if (projectChanged) { const { error: e } = await admin.from("improvement_projects").update(projectRollback).eq("id", project.id); if (e) rollbackErrors.push(`project: ${e.message}`); }
    return NextResponse.json({ error: rollbackErrors.length ? `Không ghi được audit log và rollback chưa hoàn tất: ${auditError.message}. ${rollbackErrors.join(" | ")}` : `Không ghi được audit log; thay đổi đã được hoàn tác. ${auditError.message}` }, { status: 400 });
  }
