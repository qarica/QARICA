...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const projectRecordId: null | string = null;
  ...
  if (command === "SUBMIT") {
    ...
    const id = typeof tx === "object" && tx && "project_record_id" in tx
      ? String((tx as Record<string, unknown>).project_record_id || "")
      : null;
  ...
  await admin.from("audit_logs").insert({ actor_user_id: auth.user.id, record_id: recordId, table_name: "improvement_proposals", row_id: proposal.id, action_type: `IMPROVEMENT_PROPOSAL_${command}`, old_value: { workflow_status: oldStatus }, new_value: { workflow_status: newStatus, project_record_id: projectRecordId }, reason, request_meta: { source: "qlcl-ui", transaction: "direct" } });
  return NextResponse.json({ ok: true, status: newStatus, message, project_record_id: projectRecordId, transaction: "direct" });
}
