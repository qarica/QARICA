...
export async function PATCH(request:Request,{params}:{params:Promise<{id:string}>}){
 ...
 const title=text(body.title),generalObjective=text(body.general_objective),specificObjectives=list(body.specific_objectives),requirements=text(body.requirements),leadDepartmentIds=list(body.lead_department_ids),ownerUserIds=list(body.owner_user_ids),referenceIds=list(body.reference_ids),leadDepartmentId=leadDepartmentIds[0]||text(body.lead_department_id),ownerUserId=ownerUserIds[0]||text(body.owner_user_id)||null,startDate=text(body.start_date)||null,endDate=text(body.end_date)||null,draftActions=cleanPlanDraftActions(body.draft_actions);
