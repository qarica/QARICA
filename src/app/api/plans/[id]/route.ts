...
export async function PATCH(request:Request,{params}:{params:Promise<{id:string}>}){
 ...
 const {data:program,error}=await auth.supabase.from("work_programs").select("id,record_id,workflow_status,revision_no,lead_department_id,lead_department_ids,owner_user_id,owner_user_ids,assigned_group_ids").eq("id",id).maybeSingle();
 ...
 const hasLeadDepartmentPatch=Object.prototype.hasOwnProperty.call(body,"lead_department_id")||Object.prototype.hasOwnProperty.call(body,"lead_department_ids");
 const hasOwnerPatch=Object.prototype.hasOwnProperty.call(body,"owner_user_id")||Object.prototype.hasOwnProperty.call(body,"owner_user_ids");
 const hasGroupPatch=Object.prototype.hasOwnProperty.call(body,"assigned_group_ids");
 const title=text(body.title),programType=text(body.program_type||"ANNUAL_PLAN"),generalObjective=text(body.general_objective),specificObjectives=list(body.specific_objectives),requirements=text(body.requirements),
  leadDepartmentIds=hasLeadDepartmentPatch?list(body.lead_department_ids):list(program.lead_department_ids),
  ownerUserIds=hasOwnerPatch?list(body.owner_user_ids):list(program.owner_user_ids),
  referenceIds=list(body.reference_ids),
  assignedGroupIds=hasGroupPatch?list(body.assigned_group_ids):list(program.assigned_group_ids),
  leadDepartmentId=leadDepartmentIds[0]||(hasLeadDepartmentPatch?text(body.lead_department_id):text(program.lead_department_id))||null,
  ownerUserId=ownerUserIds[0]||(hasOwnerPatch?text(body.owner_user_id):text(program.owner_user_id))||null,
  startDate=text(body.start_date)||null,endDate=text(body.end_date)||null,draftActions=cleanPlanDraftActions(body.draft_actions);
 ...
 if(!title)return NextResponse.json({error:"Cần có tên kế hoạch trước khi lưu."},{status:400});
 ...
 const [{data:validDepartments},{data:validOwners}]=await Promise.all([
  normalizedLeadDepartmentIds.length?admin.from("departments").select("id").in("id",normalizedLeadDepartmentIds).eq("organization_id",caller.organization_id).eq("is_active",true):Promise.resolve({data:[]}),
 ...
 const {error:recordUpdateError}=await admin.from("records").update({title,owner_department_id:leadDepartmentId||null,owner_user_id:ownerUserId||null}).eq("id",program.record_id);if(recordUpdateError)return NextResponse.json({error:recordUpdateError.message},{status:400});
