...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!["INVESTIGATING", "ACTION_FOLLOW_UP"].includes(String(incident.workflow_status))) {
    return NextResponse.json({ error: "Chỉ tạo CAPA sau khi sự cố đã vào bước điều tra/RCA hoặc theo dõi hành động." }, { status: 409 });
  }
  ...
  if (linkError) {
    ...
    await rollback();
    return NextResponse.json({ error: `Không liên kết được CAPA với sự cố nguồn: ${linkError.message}` }, { status: 400 });
  }
