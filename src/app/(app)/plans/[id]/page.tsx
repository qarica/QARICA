...
export default async function PlanDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: program, error: programError } = await supabase
    .from("work_programs")
    .select("id,record_id,program_type,parent_program_id,description,objective,start_date,end_date,lead_department_id,owner_user_id,workflow_status,approved_by,approved_at,created_at,updated_at")
    .eq("id", id)
    .maybeSingle();
