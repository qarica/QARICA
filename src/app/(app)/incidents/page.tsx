...
import { hcmMonthNumber, incidentAttentionRank } from "@/lib/incident-dashboard";
...
export default async function IncidentsPage(){
 ...
 const serious=rows.filter(x=>x.serious_event_flag).length,open=rows.filter(x=>!["CLOSED","CANCELLED","REJECTED"].includes(x.workflow_status)).length,investigating=rows.filter(x=>["INVESTIGATION_REQUIRED","INVESTIGATING","ACTION_FOLLOW_UP"].includes(x.workflow_status)).length;
 ...
 const months=Array.from({length:12},(_,i)=>({label:`T${i+1}`,value:0}));let missingReportedAt=0;rows.forEach(x=>{const month=hcmMonthNumber(x.reported_at);if(month===null){missingReportedAt++;return}months[month-1].value++});
 ...
 return <div className="page-stack incident-tqm">
  ...
  <section className="grid"><article className="panel"><div className="head"><h2>Xu hướng báo cáo sự cố 12 tháng</h2><p>Chỉ dùng <strong>reported_at</strong> theo múi giờ Việt Nam (Asia/Ho_Chi_Minh) — ngày nghiệp vụ báo cáo sự cố. {missingReportedAt?`${missingReportedAt} hồ sơ thiếu/không hợp lệ ngày báo cáo không được đưa vào đường xu hướng.`:"Tất cả hồ sơ đang có ngày báo cáo hợp lệ."}</p></div><TqmTrend points={months} unit=""/></article><article className="panel"><div className="head"><h2>Cơ cấu mức tổn hại</h2><p>Phân bố theo phân loại harm đã được ghi nhận.</p></div><TqmDonut value={rows.length?Math.round(serious/rows.length*100):0} label="Nghiêm trọng" segments={harmSegments.length?harmSegments:[{label:"Chưa có dữ liệu",value:1,tone:"slate"}]}/></article></section>
  ...
  <section className="panel"><div className="head"><h2>Hồ sơ cần chú ý</h2><p>Ưu tiên sự cố đang mở; trong nhóm đang mở, sự cố nghiêm trọng và đang điều tra được đưa lên trước. Hồ sơ đã đóng không lấn lên trên hồ sơ còn xử lý.</p></div><div className="hot-list">{attentionRows.slice(0,12).map(x=><div className={`hot-row ${x.serious_event_flag?"serious":""}`} key={x.id}><div><strong>{x.record_code} · {x.title}</strong><small className="subline">{x.incident_location_text||x.department}</small></div><div>{HARM_LABEL[x.harm_status]||x.harm_status||"Chưa phân loại"}</div><div><StatusBadge status={x.workflow_status}/></div><Link className="button tertiary small" href={x.route}>Mở hồ sơ</Link></div>)}</div></section>
 </div>
}