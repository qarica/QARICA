...
export default async function TaskDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [departmentRes, assigneeRes, assigneeGroupRes, groupMembershipRes, planLinkRes, evidenceRes, sourceLinksRes] = await Promise.all([
    ...
    action.assignee_group_id ? supabase.from("work_group_members").select("id").eq("group_id", action.assignee_group_id).eq("user_id", user.id).eq("is_active", true).maybeSingle() : Promise.resolve({ data: null, error: null }),
  ...
  const canOperate = action.assignee_user_id === user.id || (isGroupAssignment && !!groupMembershipRes.data) || canVerify;
  ...
  const firstError = [departmentRes as any, assigneeRes as any, assigneeGroupRes as any, groupMembershipRes as any, planLinkRes, evidenceRes, sourceLinksRes, sourceRecordsRes, { error: evidenceItemsError }].find((r: any) => r?.error)?.error;
