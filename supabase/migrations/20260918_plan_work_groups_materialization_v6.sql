...
create or replace function public.qlcl_approve_plan_bundle_v6(
  p_program_id uuid,
...
begin
  ...
  values(
    p_actor_user_id,v_record.id,'work_programs',p_program_id,'APPROVE_PLAN_BUNDLE_V6',
    jsonb_build_object(
      'program_id',p_program_id,
    ...
    jsonb_build_object('source','qlcl-ui','transaction','atomic','automation','v4','work_groups','snapshot')
  );
...
revoke all on function public.qlcl_approve_plan_bundle_v6(uuid,uuid) from public, anon, authenticated;
grant execute on function public.qlcl_approve_plan_bundle_v6(uuid,uuid) to service_role;
