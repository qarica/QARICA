...
create or replace function public.qlcl_approve_plan_bundle_v8(
  p_program_id uuid,
...
begin
    ...
    loop
      ...
      if v_monitoring_recurrence<>'ONCE' then
        ...
        insert into public.recurring_work_templates(
          ...
          lead_department_id,assignee_user_id,priority,expected_result,evidence_requirement,is_active,
        ...
        values(
          ...
          v_lead_department_id,v_assignee_user_id,v_priority,v_expected_result,v_verification_requirement,true,
  ...
  values(
    p_actor_user_id,v_record.id,'work_programs',p_program_id,'APPROVE_PLAN_BUNDLE_V8',
    jsonb_build_object(
      'program_id',p_program_id,
    ...
    jsonb_build_object('source','qlcl-ui','transaction','atomic','automation','v6_monitoring_schedule','work_groups','snapshot')
  );
...
revoke all on function public.qlcl_approve_plan_bundle_v8(uuid,uuid) from public, anon, authenticated;
grant execute on function public.qlcl_approve_plan_bundle_v8(uuid,uuid) to service_role;
