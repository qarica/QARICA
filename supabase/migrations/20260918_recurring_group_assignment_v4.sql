-- Recurring Automation V4: recurring Actions preserve USER or GROUP primary assignment.
...
create or replace function public.qlcl_materialize_recurring_run_v4(
  p_run_id uuid,
...
begin
  ...
  values(
    p_actor_user_id,v_action_record_id,'recurring_work_runs',v_run.id,'MATERIALIZE_RECURRING_RUN_V4',
...
revoke all on function public.qlcl_materialize_recurring_run_v4(uuid,uuid) from public, anon, authenticated;
grant execute on function public.qlcl_materialize_recurring_run_v4(uuid,uuid) to service_role;
