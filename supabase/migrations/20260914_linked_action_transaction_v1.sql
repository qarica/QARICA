-- QLCL-TTSG Linked Action Transaction V1
-- Creates Action Registry/domain/canonical/specialized links, notification and audit atomically.
-- PROGRAM is intentionally excluded: plan Actions use the dedicated program_action_links workflow.

begin;

create or replace function qlcl_create_linked_action_v1(
...
begin
  ...
  if v_source.record_type not in (
    'DIRECTIVE','REPORT','INSPECTION','INDICATOR_MEASUREMENT','FINDING','INCIDENT','CAPA','RISK','FMEA',
    'IMPROVEMENT_PROPOSAL','IMPROVEMENT_PROJECT','ASSESSMENT','EXTERNAL_ASSESSMENT','AUDIT','SAFETY_ALERT','FEEDBACK'
  ) then raise exception 'Source record type % does not support generic linked Action creation',v_source.record_type; end if;
  ...
  if v_source.record_type='CAPA' and v_capa_action_type not in ('CORRECTION','CORRECTIVE','PREVENTIVE','VERIFICATION') then
    raise exception 'Invalid CAPA action type';
  end if;
  if v_source.record_type='RISK' and v_risk_treatment_type not in ('AVOID','REDUCE','TRANSFER','ACCEPT','CONTINGENCY') then
    raise exception 'Invalid risk treatment type';
  end if;
  ...
  values(
    ...
    jsonb_build_object('source_record_type',v_source.record_type,'source_record_code',v_source.record_code),p_actor_user_id
  );
  ...
  values(
    ...
    jsonb_build_object(
      'action_record_id',v_action_record_id,'action_id',v_action_id,'title',v_title,
      'due_date',v_due_date,'assignee_user_id',v_assignee_user_id
    ),
    jsonb_build_object('source','qlcl-ui','source_record_type',v_source.record_type,'transaction','qlcl_create_linked_action_v1')
  );

  return jsonb_build_object(
    'ok',true,'action_id',v_action_id,'record_id',v_action_record_id,'record_code',v_record_code,'source_record_type',v_source.record_type
  );
end;
$$;

revoke all on function qlcl_create_linked_action_v1(uuid,uuid,jsonb) from public,anon,authenticated;
grant execute on function qlcl_create_linked_action_v1(uuid,uuid,jsonb) to service_role;
