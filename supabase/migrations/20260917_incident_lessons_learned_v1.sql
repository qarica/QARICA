-- QARICA incident Lessons Learned V1
-- Human-reviewed, de-identified learning publication after incident closure.
...
create table if not exists public.incident_lessons_learned (
  ...
  audience text,
  deidentified_confirmed boolean not null default false,
  review_note text,
...
create index if not exists idx_incident_lessons_status
  on public.incident_lessons_learned(status,published_at desc);

alter table public.incident_lessons_learned enable row level security;
revoke all on table public.incident_lessons_learned from public, anon, authenticated;
grant select,insert,update,delete on table public.incident_lessons_learned to service_role;
...
begin
  ...
  insert into public.incident_lessons_learned(
    ...
    v_incident.id,case when p_publish then 'PUBLISHED' else 'DRAFT' end,
    nullif(v_title,''),nullif(v_summary,''),nullif(v_learning_points,''),nullif(v_recommended_practice,''),v_audience,
    v_deidentified,v_review_note,
  ...
  on conflict(incident_id) do update set
    status=case
      when public.incident_lessons_learned.status='PUBLISHED' and not p_publish then public.incident_lessons_learned.status
    ...
    published_at=case when p_publish then coalesce(public.incident_lessons_learned.published_at,now()) else public.incident_lessons_learned.published_at end,
  ...
  insert into public.audit_logs(
    ...
    jsonb_build_object(
      'status',v_lesson.status,'deidentified_confirmed',v_lesson.deidentified_confirmed,
      'published_at',v_lesson.published_at
    ),
    case when p_publish then 'Duyệt và phát hành bài học kinh nghiệm đã xác nhận khử định danh.' else 'Lưu bản nháp bài học kinh nghiệm.' end,
    jsonb_build_object('source','qlcl-ui','transaction','qlcl_save_incident_lesson_v1')
  );

  return jsonb_build_object(
    'ok',true,'lesson_id',v_lesson.id,'status',v_lesson.status,
    'published_at',v_lesson.published_at,'deidentified_confirmed',v_lesson.deidentified_confirmed
  );
end;
$function$;

revoke all on function public.qlcl_save_incident_lesson_v1(uuid,uuid,jsonb,boolean) from public, anon, authenticated;
grant execute on function public.qlcl_save_incident_lesson_v1(uuid,uuid,jsonb,boolean) to service_role;
