-- QARICA FMEA linked Action atomic traceability V1
-- Extends qlcl_create_linked_action_v1 so an FMEA Action is created atomically with its exact failure-mode link.
begin;
...
begin
  ...
  values(
    ...
    jsonb_strip_nulls(jsonb_build_object('source_record_type',v_source.record_type,'source_record_code',v_source.record_code,'failure_mode_id',v_failure_mode_id)),p_actor_user_id
  );
  ...
  values(
    ...
    jsonb_strip_nulls(jsonb_build_object(
      ...
      'due_date',v_due_date,'assignee_user_id',v_assignee_user_id,'failure_mode_id',v_failure_mode_id
    )),
  ...
  return jsonb_strip_nulls(jsonb_build_object(
    ...
    'source_record_type',v_source.record_type,'failure_mode_id',v_failure_mode_id
  ));
end;
$$;
