-- Align Feedback runtime timestamps and make close atomic.
...
begin
  ...
  if length(v_reason) < 5 then
    raise exception 'Cần kết luận đóng phản ánh';
  end if;
  ...
  insert into public.record_status_history(
    ...
    p_feedback_record_id,v_record.lifecycle_status,'CLOSED',
  ...
  insert into public.audit_logs(
    ...
    'FEEDBACK_CLOSE',
    jsonb_build_object('workflow_status',v_feedback.workflow_status,'lifecycle_status',v_record.lifecycle_status),
    jsonb_build_object('workflow_status','CLOSED','lifecycle_status','CLOSED','closed_at',v_now),
    v_reason,
    jsonb_build_object('source','qlcl-ui','transaction','qlcl_close_feedback_v1')
  );
