...
create or replace function public.qlcl_approve_plan_bundle_v7(
  p_program_id uuid,
...
begin
  ...
  values(
    p_actor_user_id,v_record.id,'work_programs',p_program_id,'APPROVE_PLAN_BUNDLE_V7',
    jsonb_build_object(
      'program_id',p_program_id,
    ...
    jsonb_build_object('source','qlcl-ui','transaction','atomic','automation','v5_multi_output','work_groups','snapshot')
  );
...
revoke all on function public.qlcl_approve_plan_bundle_v7(uuid,uuid) from public, anon, authenticated;
grant execute on function public.qlcl_approve_plan_bundle_v7(uuid,uuid) to service_role;
